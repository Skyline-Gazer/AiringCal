...
Workflow 每个 collections 页、calendar、发布类型和 refresh chunk 都使用确定性 step 名；大 payload 写 staging KV，step 只返回 key、数量和 SHA-256 摘要。live initialize 通过 `SNAPSHOT_COORDINATOR` 分配单调 generation 并立即写 `sync:current`；refresh planning 每 10 个 subject 有界读取现有 detail、metadata、image 与 refresh 状态，只为缺失、源变化、重试到期或确定性刷新时间已到的组件生成幂等 V3 候选。Cloudflare 已验证的单次 invocation 内部 service-request 上限是 1000；planning 以 900 作为保守预算，且统计 KV 与 `SNAPSHOT_COORDINATOR` service binding。完整 chunk 最多 43 次请求。首个 invocation 从 durable prepared step output 读取总 collections 页数 `P`：前置工作为 `34 + 3P` 次请求，首组容量为 `max(0, floor((900 - (34 + 3P) - 40) / 43))` 个 chunk；若容量为零，会先在 chunk 0 前 sleep。每个 invocation 都保留 40 次请求给重入、DO 调用和终态 KV 写入；恢复后的每组固定 20 个 chunk。sleep 后从持久化 step history 重建输出，不依赖局部内存。每个 chunk 的 due 判断固定使用本次已持久化 `sync:run` 的 `started_at`，因此 sleep 期间的墙钟推进不会改变候选、预算或任务语义。候选按 new/changed、hot due、cold shard、retry 排序；普通任务受 soft limit 50 限制，只有 new/changed 可扩展到 hard limit 100。cold 候选按 `subject_id mod 7` 分散到 7 个 UTC 日。
...
成功的 manual shadow 会把固定大小、无 URL/原始 payload 的 `data.shadow` 诊断摘要写入既有 `sync:run:*` 与 `snapshot:shadow:<instance>:audit`。health 保持 `data.workflow` 优先使用 live `sync:current`；`data.shadow` 则优先读取 finalize 记录的 manual-shadow `workflow_instance_id` 对应 run，若其不是 shadow 才检查 current run，最后才兼容旧的 `sync:meta.shadow`。摘要包含 D1 行/预算计数、不可变 data R2 的 schema/generation/key/hash、是否由本次发布完成 R2 readback，以及 `public:current` 的写入计数；不会用 shared `sync:meta` 的读改写维护 diagnostics。它不证明 Cloudflare 控制面总体用量，也不改变 legacy 公开读取。D1 的 `sync_runs` 仍只保存有界计数、hash、stage/status 与分类后的 `error_code`；完整资源用量和失败诊断仍应与 Cloudflare Workflow/D1/R2/Queue/KV 控制面核对。

Worker Cron 来自 checked-in `wrangler.toml`；routine deploy 只同步代码与配置，不主动触发 live instance。

### D1 / data R2 shadow 边界

当前 D1/data R2 实现只在明确的 manual `shadow` Workflow 中运行：
...
4. 只有验证成功后才更新 KV `public:current`。pointer 只包含 `schema_version`、`generation`、`content_hash`、`r2_key` 和 `published_at`；相同已验证内容不写 R2、不增加 generation、也不写 pointer。
5. D1/R2/pointer 任一步失败都保留 pending/分类错误供同一 instance replay；不会通过回滚 D1 行或覆盖另一个 R2 generation 来“恢复”。

这个 shadow pointer 不是当前公开读取入口。`airing-cal-read` 虽已绑定 `AIRING_CAL_D1` 与 `AIRING_CAL_DATA_R2`，handler 仍只跟随 legacy KV `snapshot:active`/versioned keys（图片继续来自 `AIRING_CAL_R2`）。legacy import、`public:current` 读切换和旧 KV 清理由后续 OpenSpec change `migrate-public-reads-from-kv` 独占负责；在该 change 完成门禁前，不得把 D1/data R2 描述成公开 API 的现行来源。
