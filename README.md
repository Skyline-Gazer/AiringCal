...
成功的 manual shadow 会把固定大小、无 URL/原始 payload 的 `data.shadow` 诊断摘要写入既有 `sync:run:*` 与 `snapshot:shadow:<instance>:audit`。health 先通过现有最近 Workflow instance 标识读取对应 shadow run 的摘要；仅当该 run 不可用时，才兼容读取旧的 `sync:meta.shadow`。摘要包含 D1 行/预算计数、不可变 data R2 的 schema/generation/key/hash、是否由本次发布完成 R2 readback，以及 `public:current` 的写入计数；不会用 shared `sync:meta` 的读改写维护 diagnostics。它不证明 Cloudflare 控制面总体用量，也不改变 legacy 公开读取。D1 的 `sync_runs` 仍只保存有界计数、hash、stage/status 与分类后的 `error_code`；完整资源用量和失败诊断仍应与 Cloudflare Workflow/D1/R2/Queue/KV 控制面核对。
