...
cutover 获得 approval 后，先把 shadow manifest 引用的 immutable snapshot 复制到同名 live snapshot key（已有对象必须字节一致），再重新读取并通过 manifest/snapshot 校验；复制失败、对象冲突、缺失或 readback 校验失败都会 fail closed，不能写入 live manifest。只有 live snapshot readback 通过后才写 `public/manifest.json`。`--dry-run` 不写任何对象。运维 CLI 的 `--backup-key` 与 `--manifest-key` 只接受非空、分段的 R2 object key；scheme、userinfo、query/fragment、控制字符、空段和 `.`/`..` 段均被拒绝，凭据仍只能通过环境变量名传入。
