...
verified_scope: full-local-gates-materialized-dry-runs-v3-v4-compatible-independent-fences
verified_source_sha: 97469a62ed78769e595ef3f3ca9c3c3ed47cf0e7
---

# D1/R2 incremental sync verification-ready evidence

This report records current local evidence for runtime source SHA
`97469a62ed78769e595ef3f3ca9c3c3ed47cf0e7`. The evidence-report update after
...
|---|---|---|
| `pnpm test` | PASS | 524/524 tests, 0 failed |
...
| focused V3/V4 compatibility and fence command listed below | PASS | 129/129 tests, 0 failed |
...
`WRANGLER_LOG_PATH=/tmp/bangumitv-v4-fence-build-logs pnpm build:check` and
completed cleanly.
...
| `@airing-cal/sync-worker` | 186 |
| `@airing-cal/media-worker` | 54 |
...
Result: **129/129 passed, 0 failed**.
...
- frontend build SHA:
  `97469a62ed78769e595ef3f3ca9c3c3ed47cf0e7`.
...
| Media | D1 + legacy KV compatibility + image R2 + Queue consumer + `SubjectRefreshCoordinator`; no data R2/Workflow | PASS, 112.17 KiB / gzip 21.79 KiB |
| Sync | `SyncWorkflow` + D1 + data R2 + legacy KV compatibility + media Queue producer + `SnapshotCoordinator`; no image R2/Queue consumer | PASS, 226.65 KiB / gzip 45.67 KiB |
...
pnpm exec wrangler deploy --dry-run \
  --outdir /tmp/bangumitv-v4-fence.hIESC9/dist-<app> \
  --config /tmp/bangumitv-v4-fence.hIESC9/wrangler-<app>.toml
```
...
| D1/R2 publication is automatically verified | `.github/workflows/ci.yml`; package test scripts | full 524/524 gate plus focused 129/129 V3/V4 compatibility and fence gate | PASS |
...
## Release-blocking final review correction: independent replay-stable V4 fence
...
The final full, focused, typecheck, build, exact materialized-binding,
four-dry-run, strict OpenSpec, and diff gates are the results recorded above.
No deploy, merge, task checkoff, or production verification was performed.
