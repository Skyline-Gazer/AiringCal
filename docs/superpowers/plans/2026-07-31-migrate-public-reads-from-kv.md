...
- [ ] **Step 1: 写失败测试**（`legacy-migration.test.ts`，用 RecordingKV + SqliteD1）：
...
- [ ] **Step 3: 实现**：`readLegacySubjectRecords` 并行读四键（缺键返回 undefined）；`importLegacySubjectBatch` 对每个 subject 仅当 D1 行缺失或 `checked_at` 为空且 legacy detail 为对象时 `upsertSubjectMedia`，R2 ref 取 `imageStatus.common.r2_key`/`large.r2_key`，detail 取 `detail.subject`，nsfw 取 `meta.nsfw`；不触发 R2 PUT。
- [ ] **Step 4: 转绿**：测试 PASS；补「中断重跑不重复覆盖」「legacy 无效 JSON 计入 errored」两个测试并转绿。
- [ ] **Step 5: 提交** `git commit -m "feat(storage): import legacy subject state idempotently"`
...
- [ ] **Step 1: 失败测试**：第三批中断后重跑从持久化游标继续（构造 120 subject，前两批后 store 模拟中断，断言第二次调用从 `last_subject_id` 继续且已导入 subject 不重复计数）。
- [ ] **Step 2: 确认 FAIL**。
- [ ] **Step 3: 实现**：`runLegacyMigration` 读 cursor → `listCollectionRows()` 去重排序 → 切片批量循环 → 每批 `importLegacySubjectBatch` + cursor 推进（version=now）；批量错误捕获记 errored。
- [ ] **Step 4: 转绿** + 补「空集合零批」「批次错误不阻塞后续批次」测试。
- [ ] **Step 5: 提交** `git commit -m "feat(sync): add resumable legacy migration runner"`
...
- [ ] **Step 1: 失败测试**：仅 `published_at` 不同的两端 `equal=true`；一个业务字段差异 `equal=false` 且 streak 应重置；排序噪声（乱序 subject_id）不影响 equal；diff 不含 source_url 等敏感字段。
- [ ] **Step 2: 确认 FAIL**。
- [ ] **Step 3: 实现**（规范化 + 逐字段递归比较，稳定排序，剔除运行时键）。
- [ ] **Step 4: 转绿** + 补「calendar weekday 差异」「summary 计数差异」测试。
- [ ] **Step 5: 提交** `git commit -m "feat(storage): normalize and compare shadow public results"`
...
- [ ] **Step 1: 失败测试**：第六次差异后 streak 归零；7 次一致且预算达标返回 true；6 次一致返回 false；某日 KV 写 101 返回 false。
- [ ] **Step 2: 确认 FAIL**。
- [ ] **Step 3: 实现**：`updateShadowStreak` 用 `putAppStateIfNewer`；`shadowGatePassed` 读 streak + 最近 7 日预算 rows + `sync_budget` media 行。
- [ ] **Step 4: 转绿** + 补「streak 仅时间差异不重置」测试。
- [ ] **Step 5: 提交** `git commit -m "feat(sync): persist shadow streak and gate cutover"`
...
- [ ] **Step 1: 失败测试**：迁移期（`read-mode=legacy`）shadow 发布只写 `public:shadow-current`，`public:current` 不变；门禁通过后 `promoteShadowPointer` 写 `public:current` + `public:read-mode=r2` 且顺序为 pointer 最后。
- [ ] **Step 2: 确认 FAIL**。
- [ ] **Step 3: 实现**：r2-publication pointer key 参数化；d1-sync shadow 路径在 `read-mode` 非 r2 时用 shadow key；`promoteShadowPointer` 验证后写 pointer 再镜像 read-mode（KV 失败不抛，记录 degraded）。
- [ ] **Step 4: 转绿** + 补「提升幂等：重复调用不重复写 pointer」测试。
- [ ] **Step 5: 提交** `git commit -m "feat(sync): gate public pointer behind shadow gate"`
...
- [ ] **Step 1: 失败测试**：未知 schema 拒绝并使用 fallback；R2 GET 失败但缓存存在返回缓存版本；缓存与 R2 均失败回 legacy；`/api/collections` 分页结果与现契约一致（type/page/limit/cursor）。
- [ ] **Step 2: 确认 FAIL**。
- [ ] **Step 3: 实现**：pointer 校验（schema=1、generation≥0、hash 64 hex、r2_key、published_at）；R2 GET `snapshots/v1/{generation}-{hash}.json` → `parsePublicSnapshotV1`；Cache API key `r2-snapshot:{hash}` 存完整对象 + 验证元数据；hydration 不再走 legacy 逐 subject KV。
- [ ] **Step 4: 转绿** + 补「图片引用与 NSFW 直接来自 snapshot」测试。
- [ ] **Step 5: 提交** `git commit -m "feat(read): load verified R2 snapshot with bounded fallback"`
...
- [ ] **Step 1: 失败测试**：mode=legacy 时 health 返回 `snapshot.source='legacy'` 且既有字段全保留；mode=r2 时 `snapshot.source='r2'` 且 generation 正确；D1 不可读时 health 仍 200 且 `degraded:true`。
- [ ] **Step 2: 确认 FAIL**。
- [ ] **Step 3: 实现**：接线 + health 构造。
- [ ] **Step 4: 转绿** + 全仓 `CI=true pnpm test` 与 `pnpm typecheck` PASS。
- [ ] **Step 5: 提交** `git commit -m "feat(read): switch read source and expose migration health"`
...
- [ ] **Step 1: 失败测试**：切换第 10 天删除 0；第 15 天 ≤100；失败批次不推进游标；至少保留一个已验证 generation（删除前校验 R2 key）。
- [ ] **Step 2: 确认 FAIL**。
- [ ] **Step 3: 实现**：游标 + KV delete + 校验。
- [ ] **Step 4: 转绿**。
- [ ] **Step 5: 提交** `git commit -m "feat(sync): rate-limited legacy cleanup after 14 days"`
...
- [ ] **Step 1: 失败测试**：回滚后 read-worker 走 legacy 路径、R2 对象仍存在、API 契约不变；重复回滚幂等。
- [ ] **Step 2: 确认 FAIL** → Step 3 实现 → Step 4 转绿。
- [ ] **Step 5: 提交** `git commit -m "docs(sync): rollback read mode with preserved R2 generations"`
...
- [ ] **Step 1: 失败测试**：cron 20:00 同时执行 legacy 与 shadow 两阶段；shadow 阶段抛错后 legacy 仍成功且 run 记录错误；非 20:00 的 cron 仍跳过。
- [ ] **Step 2: 确认 FAIL** → Step 3 实现 → Step 4 转绿（含既有 scheduled/queue 测试全过）。
- [ ] **Step 5: 提交** `git commit -m "feat(sync): run daily shadow migration and gate in scheduled workflow"`
...
- [ ] **Step 1: 更新文档**（README 各段 + tasks.md 勾选）。
- [ ] **Step 2: 全仓门禁**：`CI=true pnpm test`、`pnpm typecheck`、`pnpm build:check`、`./node_modules/.bin/openspec validate migrate-public-reads-from-kv --strict`、`git diff --check` 全部 PASS。
- [ ] **Step 3: 审查**：按 `review_mode` 完成代码审查并修复 CRITICAL 发现。
- [ ] **Step 4: 提交** `git commit -m "docs(migrate): synchronize architecture and verification evidence"`
