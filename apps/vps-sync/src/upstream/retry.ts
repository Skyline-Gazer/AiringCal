...
function classify(error: unknown): Classification {
  if (error instanceof BgmHttpError) {
    if (error.status === 401 || error.status === 403) return { category: 'auth', code: error.status === 401 ? 'UPSTREAM_UNAUTHORIZED' : 'UPSTREAM_FORBIDDEN', retry: false }
    if (error.status === 404) return { category: 'not_found', code: 'UPSTREAM_NOT_FOUND', retry: false }
    if (error.status === 429) return { category: 'rate_limited', code: 'UPSTREAM_RATE_LIMITED', retry: true, retryAfter: error.retryAfter }
    if (error.status >= 500) return { category: 'upstream', code: 'UPSTREAM_5XX', retry: true, retryAfter: error.retryAfter }
...
function retryAfterDelay(value: string | undefined, now: number, maximum: number): number | null {
  ...
  if (Number.isFinite(parsed)) return Math.min(maximum, Math.max(0, parsed - now))
  return null
}
...
export async function withRetry<T>(operation: (attempt: number) => Promise<T>, policy: RetryPolicy): Promise<T> {
  ...
  if (!Number.isSafeInteger(maxAttempts) || maxAttempts < 1 || maxAttempts > 3 || !Number.isFinite(baseDelayMs) || baseDelayMs < 0 || !Number.isFinite(maxDelayMs) || maxDelayMs < 0) {
