...
import { refreshMedia, type MediaDependencies } from './refresh.ts'
...
function fixture(current: MediaResultInput | null = null) {
  const events: string[] = []
  const saves: MediaResultInput[] = []
  const deps: MediaDependencies = {
    list: async () => [{ subjectId: 1, priority: 'new_or_changed' }],
    withSubject: async (_id, work) => {
      events.push('lock')
      try { return await work({ current, save: async (value) => { events.push('save'); saves.push(value); return true } }) }
...
function stored(overrides: Partial<MediaResultInput> = {}): MediaResultInput {
  return { subjectId: 1, runId: 'old', observedAt: '2026-08-01T00:00:00.000Z', detail: { id: 1, name: 'old' }, metadata: null,
...
test('detail 404 keeps successful data and sets bounded tombstone; TTL skips upstream', async () => {
  const { deps, saves } = fixture(stored())
  deps.detail = async () => null
  await refreshMedia(deps, context)
  assert.equal(saves[0]?.detail, null)
  assert.equal(saves[0]?.metadata?.exists, false)
  assert.equal(Date.parse(saves[0]!.nextRetryAt!) - Date.parse(at), 86400000)
...
test('rejects invalid image MIME, HTTP, size and unsafe URLs before storing', async () => {
  for (const response of [new Response('html', { headers: { 'content-type': 'text/html' } }), new Response('x', { status: 403 }), new Response(new Uint8Array(8 * 1024 * 1024 + 1), { headers: { 'content-type': 'image/png' } })]) {
...
test('bounded concurrency drains all in-flight work even if a save fails', async () => {
  ...
  deps.list = async () => Array.from({ length: 20 }, (_, i) => ({ subjectId: i + 1, priority: 'hot' }))
  deps.withSubject = async (_id, work) => {
    active++; maximum = Math.max(maximum, active)
    try { await Promise.resolve(); return await work({ current: null, save: async () => { throw new Error('DB') } }) }
    finally { active-- }
  }
