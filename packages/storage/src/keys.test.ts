...
import { imageIndexKey, imageOriginalKey, imageStatusKey, snapshotActiveKey, snapshotCollectionsKey, snapshotVersionKey, subjectDetailKey, subjectMetaKey, subjectRefreshKey, syncCurrentKey, syncRunKey, syncShadowKey, syncStagingKey } from './index.ts'
...
test('storage key builders expose stable KV and R2 contracts', () => {
  ...
  assert.equal(syncCurrentKey(), 'sync:current')
  assert.equal(snapshotActiveKey(), 'snapshot:active')
  assert.equal(snapshotVersionKey('instance-1', 'collections:watched'), 'snapshot:version:instance-1:collections:watched')
