...
4. The Read Worker validates manifest schema, key shape, and content hash. It rejects a lower generation or same-generation hash conflict only when the current request can read and re-validate the complete `last-verified` `{ manifest, snapshot }` envelope from Cache API; on R2 failure it uses that verified envelope, then the complete legacy KV snapshot during migration. Cache API contents are local to the originating data center and may be absent, expire, or be evicted. The write queue is per Worker isolate, and Cache API provides no shared atomic compare-and-swap across isolates, so rollback protection across isolates/points of presence is best effort and has no global monotonic-generation guarantee. The browser never receives storage/database credentials.
5. A successful publication is followed by custom-format `pg_dump` to private R2. Backup failure yields partial success and never rolls back a public snapshot. Retention selection identifies the newest 30 daily backups plus the last backup of each earlier month; this plan reports candidate keys only and does not issue R2 deletes. Actual object deletion requires a separately approved OpenSpec change.
